...
3. **Score, not pass/fail — but not consequence-free either.** There's still no big binary "YOU LOSE" screen, and the player always gets a score and share card, even on1 an early lives-out ending. But running out of lives is a real, meaningful outcome — the player doesn't get to see or attempt the rest of the pool. It's closer to Wordle's "you get one board, and it can end before you've seen everything" than a purely forgiving model, and the doc should be honest about that rather than glossing over it.
