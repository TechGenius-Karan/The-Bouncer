...
5. **Live for real review sessions.** Not started — this is an operational step (accumulate `aiReviews` data, then hand-revisit the system prompt / grow §5's few-shot library), not code. **Prerequisite still outstanding from phase 1**: run `npm run content:seed-db` so the live `rules` collection carries all 29 rules (currently 17) — the override/retire controls only cover rules that exist as documents.
