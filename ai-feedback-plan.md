...
|---|---|---|---|
| **Google Gemini 2.5 Flash** (💡 recommended) | Yes, indefinite, no card | 10 RPM / 250 requests per day, 250k TPM shared across free-tier models | Strong structured-output support (`responseSchema`), capable general reasoning, Google's own SDK |
...
**Pick: Gemini 2.5 Flash's free tier**, with Groq as the documented fallback if Gemini's daily cap or terms ever become a problem, and Claude Haiku 4.5 as a cheap paid escalation path if either free tier's reasoning quality proves insufficient in practice.
...
1. **Schema + manual override plumbing, no AI yet.** `RuleDoc.disabled`/`subtletyOverride` fields, `ruleOverrides.ts` + its tests, wiring into the three generation call sites, `admin-rule-override.ts` endpoint, and the `RuleRejectStatsPanel.tsx` manual retire/reinstate/adjust controls (§8). This alone already delivers the "reviewer can straight-up just reject the entire rule/concept" half of the original request, independent of AI, and is fully testable/shippable on its own.
2. **`aiReviews` collection + `aiReview.ts` module, backend only.** Build and manually exercise via a script (mirroring how `queuePuzzles.ts`/`tagWords.ts` are CLI-testable outside the app) before wiring any HTTP endpoint — confirm real Gemini calls return sensibly-structured decisions against a handful of real pending puzzles.
3. **`admin-ai-review.ts` endpoint**, dispatch logic extracted and tested per §10, wired to the existing `repairWord.ts`/`generateCandidate`/`admin-rule-override.ts` machinery from steps 1–2.
