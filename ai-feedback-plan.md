...
|---|---|---|---|
| **Google Gemini Flash (free tier)** — currently `gemini-3.6-flash`, see the model-journey note below | Yes, no card | Moves fast; verify per-model at the rate-limits page. 3.6-flash returns a structured call in ~5s in practice | Strong structured-output support (`responseSchema`), capable general reasoning, Google's own SDK |
| Groq (Llama 3.3 70B / GPT-OSS-120B / etc.) | Yes, indefinite, no card | 30 RPM / 6,000 TPM / 14,400 requests per day | Fastest inference, most generous free daily cap by far, hosts strong open-weight models — the documented fallback if Gemini's free access keeps churning |
| Anthropic Claude Haiku 4.5 | No lasting free tier (small one-time signup credit only) | Pay-per-use: ~$1/$5 per million input/output tokens | Excellent structured reasoning; pennies per review at this volume — the paid escalation if free-tier quality/quota disappoints |
| OpenAI | No lasting free tier | Comparable pay-per-use pricing to Claude Haiku | No particular advantage over Claude here |

**Pick: Gemini's free tier** (`gemini-3.6-flash`), with Groq as the fallback and Claude Haiku 4.5 as the paid escalation.

**Model journey — a cautionary tale about trusting docs over the live API (2026-08-30):** the free-tier Flash lineup churned *hard* during this build. The plan first named `gemini-2.5-flash`; then third-party sources said `gemini-3.5-flash` was free with 1,500 req/day, so we switched. Real use exposed both claims as wrong: **3.5-flash's actual free cap is 20 requests/day** (confirmed by a live 429: `limit: 20`) and it ran ~20s+ per call; and **2.5-flash now 404s for new accounts** ("no longer available to new users"). Google's own 404 message steered us to `gemini-3.6-flash`, which works (~5s, structured output good). It also rejects `thinkingConfig.thinkingBudget: 0` with a 400, so we send no thinking config. **Lesson: verify the model name, quota, and accepted params against the live API, not blog posts — and keep the provider behind one small interface (§7.4) so the next inevitable churn is a one-line change.**

Reasoning: this feature fires only when a reviewer clicks reject — realistically single digits to low tens of calls per day. The model's reasoning quality is well past what's needed to read a short puzzle + feedback and either pick a bounded action or author replacement words from a menu. Cost stays $0 at this volume.
...
> **Revision — Option A (2026-08-30), after real use.** The reviewer hit a hidden-number puzzle that kept using only "ten"/"one" words, wrote detailed feedback with two example fixes, and watched the AI *correctly understand it in the rationale* while the puzzle didn't change — because the original `redraft-puzzle` action was a **blind re-roll** of the same deterministic generator, which has no way to receive the reviewer's content guidance (and the word bank is 86% "one"/"ten" words, so a re-roll reproduces the skew). The fix, chosen by the user: **replace `redraft-puzzle` with `rewrite-puzzle`, where the AI authors the actual replacement words.** This reverses the original "no LLM puzzle editing" stance (§0) deliberately — the safety comes from the AI choosing only from a menu of real, correctly-sided bank words, the server recomputing every label from the true rule (never trusting the AI's), and the full uniqueness validator gating the result before it's accepted. Verified end-to-end: the exact hidden-number complaint now produces a puzzle spanning nine/two/ten/one. The table and `AiReviewAction` below reflect this revision.
...
// AiAuthoredWord = { word: string; label: 'IN' | 'OUT' }
```
...
| `rewrite-puzzle` | The rule is fine but the **word choices** need to change to honor the feedback (more variety, better/harder examples). The AI authors the new words itself | `aiReviewDispatch.ts::validateAuthoredPuzzle` — AI picks only from a server-provided menu of real, correctly-sided words; server recomputes all labels from the rule, enforces knob counts + a real IN/OUT mix, then runs the uniqueness validator; invalid → falls back to reject |
...
4. ✅ **Frontend.** Done 2026-08-30. `PuzzleReviewCard.tsx` — the Phase 10.6 word-picker dropdown is gone; Reject is now a single free-text reason that routes to `admin-ai-review` (the AI infers word-vs-concept itself), plus a separate low-emphasis "Retire this rule (no AI)" link with a confirmation dialog. `AdminApp.tsx` — `handleReject` calls `aiReview` and shows a dismissible banner at the top of the screen with the AI's action + rationale (colored green when the puzzle survived and is back in the queue, neutral when it was rejected), then reloads; `handleRetireRule` retires the rule *and* rejects this instance (via the plain no-AI `admin-reject`, since its rule is now gone) so it doesn't linger under a disabled rule.
   - **Dead code removed** (this plan's §0 supersession): the now-orphaned `admin-repair-word.ts` HTTP endpoint, its `repairWord()` client function, and the `AdminRepairWordRequest`/`Response` wire types from both contract files. The *pure* `content-engine/generator/repairWord.ts` stays — the AI's `swap-word` action calls it directly through `aiReviewDispatch`.
   - **Verified**: typecheck clean (all three trees), full suite 341/341, lint clean, full production build succeeds.
   - **Not verified — needs a manual pass**: the actual click-through of the gated admin UI. The admin screen requires an access code, and entering an authentication credential through browser automation is a prohibited action regardless of source, so the visual/interaction check (banner rendering, retire confirmation, reason submission) is the one thing left for a human to eyeball before relying on it in a real session.
4a. ✅ **Option A revision — `rewrite-puzzle` replaces `redraft-puzzle`.** Done 2026-08-30, after the reviewer found redraft was a useless blind re-roll (see §6's revision note). The AI now authors the replacement words itself from a server-provided menu of real, correctly-sided bank words (`admin-ai-review.ts` builds a 100-IN / 40-OUT shuffled menu, excluding words already in the puzzle so skewed rules like hidden-number still surface their rare words); `aiReviewDispatch.ts::validateAuthoredPuzzle` gates the result hard — words must exist in the bank, clue labels must match the real rule, counts must match the tier knobs, guest labels are recomputed from the rule (never trusted from the AI), the pool must be a genuine IN/OUT mix, and the whole thing runs the uniqueness validator; anything off → falls back to reject. Frontend banner headline updated for rewrite (success vs. didn't-validate). Response schema gained `clues`/`guests` arrays; `parseAiReviewAction` shape-validates them.
   - **Verified end-to-end against the exact failing case**: the reviewer's real hidden-number complaint ("keeps using only ten/one, vary it") now yields a rewrite spanning **nine (canine), two (twosome/twofold), ten (straighten/tension), one (honey)** — genuine variety, validator-passed, back in the queue. ~13s call, within the 25s timeout. Typecheck (3 trees), 345/345 tests (rewrite replaced the redraft tests + added 5 validation-guard cases), lint, and production build all clean.
   - **Latency/timeout note**: `gemini-3.6-flash` structured calls run ~5-13s; timeout is 25s (Netlify sync functions cap at 26s). If a future model runs slower, this feature could brush that cap in production — watch for it, and consider a faster model or Netlify background functions if it becomes real.
5. **Live for real review sessions.** Not started — this is an operational step (accumulate `aiReviews` data, then hand-revisit the system prompt / grow §5's few-shot library), not code. **Prerequisite from phase 1 now DONE**: `npm run content:seed-db` was run — the live `rules` collection carries all 29 rules.
