...
export async function resolveClueWords(puzzle: PuzzleDoc): Promise<{ in: string[]; out: string[] }> {
  const { words } = await getCollections()
  const wordDocs = await words.find({ _id: { $in: puzzle.clues.map((c) => c.wordId) } }).toArray()
  const spellingOf = new Map(wordDocs.map((w) => [w._id, w.spelling]))

  return {
    in: puzzle.clues.filter((c) => c.label === 'IN').map((c) => spellingOf.get(c.wordId) ?? c.wordId),
    out: puzzle.clues.filter((c) => c.label === 'OUT').map((c) => spellingOf.get(c.wordId) ?? c.wordId),
  }
}
