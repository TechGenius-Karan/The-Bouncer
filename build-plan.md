...
- [ ] **Step 6 — Wire in.** `RULES = [...LEXICAL_RULES, ...SEMANTIC_RULES]` in `content-engine/rules/index.ts`.
- [ ] **Step 7 — Difficulty mix.** `difficulty.ts` knob for the lexical/semantic split, starting at [§7.1](planning.md#71-starter-rule-taxonomy)'s own suggested ~70/30 — also the lever for deprioritizing specific lexical rules if playtesting keeps flagging them as too obscure or too easy to pass by chance.
- [ ] **Step 8 — Validate.** Generate a fresh batch (`npm run content:generate`), confirm the existing validator/decoy-scan tests still pass against the expanded taxonomy, eyeball a handful of generated semantic puzzles for quality.
